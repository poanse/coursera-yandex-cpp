...
struct BulkTaxApplier {
  ...
  //uint32_t count = 0;
	double factor = 1;
	BulkTaxApplier(size_t percent = 0) {
		update(percent);
	}

	void update(size_t percent) {
		if (percent != 0) {
...
public:
  ...
  void CombineWith(const BulkLinearUpdater& other) {
    //tax_.count += other.tax_.count;
		...
		add_.spending += other.add_.spending;
  ...
  pair<double, double> Collapse(pair<double, double> origin, IndexSegment segment) const {
    return { origin.first * tax_.ComputeFactor() + add_.income * segment.length(), 
						 origin.second + add_.spending * segment.length() };
  }
...
struct ComputeIncomeRequest : ReadRequest<double> {
  ...
  double Process(const BudgetManager& manager) const override {
    const auto [income, spending] = manager.ComputeSum(MakeDateSegment(date_from, date_to));
...
struct SpendRequest : ModifyRequest {
  ...
  void Process(BudgetManager& manager) const override {
    ...
    manager.AddBulkOperation(date_segment, BulkMoneyAdder{0, daily_spending});
  }
...
struct PayTaxRequest : ModifyRequest {
  ...
  void ParseFrom(string_view input) override {
    date_from = Date::FromString(ReadToken(input));
    date_to = Date::FromString(ReadToken(input));
		tax_percent = ConvertToInt(input);
  }

  void Process(BudgetManager& manager) const override {
    manager.AddBulkOperation(MakeDateSegment(date_from, date_to), BulkTaxApplier(tax_percent));
  }
