...
array<double, VERTEX_COUNT> tree_values, tree_add, tree_factor, 
	tree_total_spend, tree_delta_spend;

void Init() {
  tree_values.fill(0);
  tree_add.fill(0);
  tree_total_spend.fill(0);
  tree_delta_spend.fill(0);
  tree_factor.fill(1);
}
  ...
  for (size_t w = v * 2; w <= v * 2 + 1; ++w) {
    if (w < VERTEX_COUNT) {
      tree_factor[w] *= tree_factor[v];
			tree_add[w] = tree_add[w] * tree_factor[v] + tree_add[v];
      tree_values[w] = tree_values[w] * tree_factor[v] + tree_add[v] * (r - l) / 2;
			tree_delta_spend[w] += tree_delta_spend[v];
			tree_total_spend[w] += tree_delta_spend[v] * (r - l) / 2;
    }
  }
  tree_factor[v] = 1;
  tree_add[v] = 0;
	tree_delta_spend[v] = 0;
}
...
double ComputeSum(size_t v, size_t l, size_t r, size_t ql, size_t qr) {
  ...
  Push(v, l, r);
  if (ql <= l && r <= qr) {
    return tree_values[v] - tree_total_spend[v];
  }
...
void Spend(size_t v, size_t l, size_t r, size_t ql, size_t qr, double value) {
  ...
  Push(v, l, r);
  if (ql <= l && r <= qr) {
    tree_delta_spend[v] += value;
		tree_total_spend[v] += value * (r - l);
    return;
  }
  Spend(v * 2, l, (l + r) / 2, ql, qr, value);
  Spend(v * 2 + 1, (l + r) / 2, r, ql, qr, value);
	tree_total_spend[v] =
			(v * 2 < VERTEX_COUNT ? tree_total_spend[v * 2] : 0)
			+ (v * 2 + 1 < VERTEX_COUNT ? tree_total_spend[v * 2 + 1] : 0);
}
