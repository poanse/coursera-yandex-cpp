...
struct AddressHasher {
  // реализуйте структуру
	...
	size_t operator()(const Address& address) const {
		auto r1 = sh(address.city);
		auto r2 = sh(address.street);
		auto r3 = ih(address.building);
		size_t x = 2'946'901;
...
struct PersonHasher {
  // реализуйте структуру
	...
	size_t operator()(const Person& person) const {
		auto r1 = sh(person.name);
		auto r2 = ih(person.height);
		auto r3 = dh(person.weight);
		auto r4 = ah(person.address);
		size_t hash = 2'946'901;
		hash += hash * r1;
		hash += hash * r2;
		hash += hash * r3;
		hash += hash * r4;
