...
void TestSearchServer(vector<pair<istream*, ostream*>> streams) {
    ...
    for (auto& [input, output] : IteratorRange(begin(streams) + 1, end(streams))) {
        this_thread::sleep_for(random_time());
				//cerr << "thread launched" << endl;
        if (!output) {
						//cerr << "Update start attempted" << endl;
            srv.UpdateDocumentBase(*input);
						//cerr << "Update start successfull" << endl;
        } else {
						//cerr << "AddQueries start attempted" << endl;
            srv.AddQueriesStream(*input, *output);
						//cerr << "AddQueries start successfull" << endl;
        }
    }
		//cerr << "All threads launched" << endl;
}
...
void TestMultipleThreads() {
  ...
  const vector<string> queries = {
	...
	istringstream is0(Join('\n', docs));
	istringstream is1(Join('\n', queries));
	istringstream is2(Join('\n', queries));
	ostringstream os1;
	ostringstream os2;
	pair<istringstream*, ostringstream*> p0 = 
		make_pair<istringstream*, ostringstream*>(&is0, nullptr);
	pair<istringstream*, ostringstream*> p1 = 
		make_pair<istringstream*, ostringstream*>(&is1, &os1);
	pair<istringstream*, ostringstream*> p2 =
		make_pair<istringstream*, ostringstream*>(&is2, &os2);
	vector<pair<istream*, ostream*>> streams {p0, p1, p2};

	//LOG_DURATION("Multiple streams");
...
int main() {
	...
	RUN_TEST(tr, TestMultipleThreads);
	...
	for (auto i = 0; i < 100000; i++) {
		queries.push_back("daddy");
		queries.push_back("need");
	}
	istringstream docs_input(Join('\n', docs));
	istringstream queries_input(Join('\n', queries));
	cerr << "TestDuration:" << endl;
	LOG_DURATION(" Total");
	SearchServer srv;
	{
